...
- For each baseline: clone official repo as reference → extract and adapt model code into the unified project's model module, conforming to the common model interface. Write a config file for the baseline. If following an existing codebase, respect its conventions.
- Run training/evaluation using the unified entry points with each baseline's config file
...
- Run initial training with the architecture delivered by the strategist (Step 3.2) using the unified entry points with our method's config file
...
- Execute ablation studies via project-level scripts (e.g., `scripts/ablation.py`) — Step 3.5
- Run multi-seed experiments for statistical significance — Step 3.6
- Execute claim-proof experiments via project-level scripts (e.g., `scripts/claim_proof.py --claim <id>`) — Step 3.7
...
- Update `README.md` with final reproduction commands for all methods
...
- Update `README.md` on the remote server with final reproduction commands (Step 4.6)
